...
class Auth
{
    ...
    public function check($name, $uid, $relation = 'or', $mode = 'url')
    {
        ...
        $uid = $uid?$uid:session('admin.id');
        
        if (SYS_ADMINISTRATOR_ID ==  $uid)
        {
            return true;
        }
    ...
    public function getRuleList($uid)
    {
        ...
        if (SYS_ADMINISTRATOR_ID !=  $uid && !in_array('*', $ids))
        {
            $where['id'] = $ids;
        }
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->cache(true)->field('id,pid,condition,icon,name,title,ismenu')->select();
        ...
        {
            //超级管理员无需验证condition
            if (!empty($rule['condition']) && SYS_ADMINISTRATOR_ID !=  $uid)
            {
                //根据condition进行验证
    ...
    protected function getUserInfo($uid)
    {
        ...
        {
            $user_info[$uid] = $user->where($_pk, $uid)->cache(true)->find();
        }
