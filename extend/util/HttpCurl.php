 ...
 * @DateTime 2017-07-07
 * 例 echo (new HttpCurl())->setParams(['name' => 'dfh', 'age' => 12])->get('http://www.test.com');
 */
namespace util;
...
class HttpCurl
{
     ...
     * 
     * $jsonObj = json_encode($listArr);
     * $list    = HttpCurl::instance()->setParams($jsonObj)->setHeader(["Content-Type:application/json"])->post('http://192.168.92.118:8080/api/crawler/checkout', 'json');
     */
    public function post($url, $dataType = 'text')
    {
        if (stripos($url, 'https://') !== false) {
            curl_setopt($this->ch, CURLOPT_SSL_VERIFYPEER, false); //// https请求 不验证证书和hosts
            curl_setopt($this->ch, CURLOPT_SSL_VERIFYHOST, false);
            curl_setopt($this->ch, CURLOPT_SSLVERSION, 1);
        }
        curl_setopt($this->ch, CURLOPT_URL, $url);
